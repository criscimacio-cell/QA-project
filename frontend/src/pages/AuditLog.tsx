...
const ACTION_STYLES: Record<string, string> = {
  ...
  VIEW: 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-500',
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Security and compliance event history</p>
        </div>
      ...
      <div className="grid grid-cols-5 gap-3">
        {summary.map(s => (
          <div key={s.action} className="card p-3 text-center">
            ...
            <div className={`text-xs mt-0.5 font-medium px-2 py-0.5 rounded-full inline-block ${ACTION_STYLES[s.action] || 'bg-gray-100 text-gray-600'}`}>{s.action}</div>
          </div>
        ))}
      </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {logs.map(l => (
                  <tr key={l.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap font-mono">{new Date(l.created_at).toLocaleString()}</td>
                    <td className="px-4 py-3">
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{l.user_name || 'System'}</div>
                      <div className="text-xs text-gray-400">{l.user_email}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs capitalize text-gray-500 dark:text-gray-400">{l.user_role}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${ACTION_STYLES[l.action] || 'bg-gray-100 text-gray-600'}`}>{l.action}</span>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400 capitalize">{l.entity_type}</td>
                    <td className="px-4 py-3 text-xs text-gray-600 dark:text-gray-300 max-w-[240px] truncate" title={l.details}>{l.details}</td>
                    <td className="px-4 py-3 text-xs text-gray-400 font-mono">{l.ip_address}</td>
                  </tr>
                ))}
                {logs.length === 0 && (
                  <tr><td colSpan={7} className="px-4 py-12 text-center text-gray-400 text-sm">No audit events found</td></tr>
                )}
              </tbody>
            </table>
          </div>
