  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
      ...
      <div className="card overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-16"><RefreshCw size={20} className="animate-spin text-[#7C3AED]" /></div>
        ) : files.length === 0 ? (
                  ...
                  <tr key={f.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        {isPreviewable(f) && (
                          <button onClick={() => openPreview(f)} title="Preview" className="p-1.5 rounded hover:bg-[#7C3AED]/10 text-[#7C3AED]">
                        ...
                        {f.status === 'archived' ? (
                          ...
                          <>
                            <button onClick={() => { setSelected(f); setShowApprove(true); }} title="Review" className="p-1.5 rounded hover:bg-[#7C3AED]/10 text-[#7C3AED]">
                              <GitBranch size={14} />
                            </button>
                ...
                <div className="space-y-2">
                  {selected.versions.map((v: any) => (
                    <div key={v.id} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-sm">
                      <span className="text-xs bg-[#7C3AED]/10 text-[#7C3AED] px-2 py-0.5 rounded font-mono">v{v.version}</span>
      ...
      <Modal open={showBulkUpload} onClose={() => { setShowBulkUpload(false); setBulkFiles([]); setBulkProgress(''); }} title="Bulk Upload Files" size="md">
        <div className="space-y-4">
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${dragOver ? 'border-[#7C3AED] bg-[#7C3AED]/5' : 'border-slate-200 dark:border-slate-700 hover:border-[#7C3AED]/50'}`}
          >
          ...
          {bulkProgress && (
            <p className={`text-sm text-center font-medium ${bulkProgress.includes('complete') ? 'text-[#7C3AED]' : bulkProgress.includes('fail') ? 'text-red-500' : 'text-slate-600 dark:text-slate-400'}`}>
              {bulkProgress}
            </p>
          )}
