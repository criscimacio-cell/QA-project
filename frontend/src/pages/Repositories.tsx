    ...
    <div>
      ...
      >
        ...
        {shouldOpen && hasChildren
          ...
          : <Folder size={15} className={`flex-shrink-0 ${level === 0 ? 'text-[#7C3AED]' : 'text-amber-400'}`} />
        }
            ...
            {menuOpen && (
              <div className="absolute right-0 top-5 z-50 w-44 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 py-1 text-xs animate-scale-in">
                <button onClick={() => { onAddSub(node); setMenuOpen(false); }}
                  ...
                  <FolderPlus size={13} className="text-[#7C3AED]" /> Add Sub-folder
                </button>
      ...
      <div className="w-64 flex-shrink-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col">
        ...
        <div className="px-3 pt-4 pb-2 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              ...
              {isLead && (
                <button
                  ...
                  className="p-1 rounded hover:bg-[#7C3AED]/10 text-[#7C3AED] transition-colors"
                >
                  <Plus size={14} />
                </button>
              )}
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-7 pr-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 placeholder-slate-400 outline-none focus:border-[#7C3AED] transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
          {tree.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-slate-400 gap-2">
              ...
              {isLead && (
                <button onClick={() => openCreate(null)} className="text-xs text-[#7C3AED] hover:underline font-medium">
                  + Create one
                </button>
              )}
            </div>
        ...
        {isLead && (
          <div className="flex-shrink-0 p-3 border-t border-slate-100 dark:border-slate-800">
            <button
              onClick={() => openCreate(null)}
              className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium text-[#7C3AED] border border-dashed border-[#7C3AED]/40 hover:border-[#7C3AED] hover:bg-[#7C3AED]/5 transition-all"
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
        ...
        <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-5 py-2.5 flex items-center gap-3 flex-wrap flex-shrink-0">
            ...
            {breadcrumb.length === 0 ? (
              ...
              <span key={i} className="flex items-center gap-1 text-xs">
                ...
                <span className={i === breadcrumb.length - 1
                  ...
                  : 'text-slate-400 hover:text-[#7C3AED] cursor-pointer transition-colors'}>
                  {b}
                </span>
              </span>
            ))}
          ...
          <div className="flex items-center border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
            <button onClick={() => setView('list')} className={`p-1.5 transition-colors ${view === 'list' ? 'bg-[#7C3AED] text-white' : 'bg-white dark:bg-slate-800 text-slate-400 hover:text-slate-600'}`}><List size={14} /></button>
            <button onClick={() => setView('grid')} className={`p-1.5 transition-colors ${view === 'grid' ? 'bg-[#7C3AED] text-white' : 'bg-white dark:bg-slate-800 text-slate-400 hover:text-slate-600'}`}><LayoutGrid size={14} /></button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {!selected ? (
            ...
            <div className="flex items-center justify-center h-32">
              <RefreshCw size={18} className="animate-spin text-[#7C3AED]" />
            </div>
          ) : (
            <>
              {/* Repo info header */}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    {repoDetail.children.map((c: any) => (
                      <div
                        ...
                        className="card p-3 cursor-pointer hover:border-[#7C3AED]/40 hover:shadow-md transition-all group"
                      >
                    ...
                    {isLead && (
                      <button
                        ...
                        className="card p-3 flex flex-col items-center justify-center gap-1.5 border-dashed cursor-pointer hover:border-[#7C3AED]/50 hover:bg-[#7C3AED]/5 transition-all text-slate-400 hover:text-[#7C3AED]"
                      >
                        <Plus size={20} />
  ...
  return (
    <Modal open={open} onClose={onClose} title="Upload File" size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div
          className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${uploadErrors.file ? 'border-red-400' : 'border-slate-200 dark:border-slate-700 hover:border-[#7C3AED]/50'}`}
        >
          {file ? (
            ...
            <label className="cursor-pointer">
              ...
              <p className="text-sm text-slate-500 mb-1">Drop a file here or <span className="text-[#7C3AED] font-medium">browse</span></p>
