...
export default function BackofficeLogin() {
  ...
  useEffect(() => {
    if (!authLoading && isAuthenticated) navigate('/backoffice', { replace: true });
  ...
  const triggerScratchHead = () => {
    ...
    try { splineRef.current.emitEvent('mouseDown', 'Robot'); } catch {}
  };
  ...
  useEffect(() => {
    ...
    ts.push(setTimeout(() => navigate('/backoffice'), 2100));
    return () => ts.forEach(clearTimeout);
  }, [loginSuccess, navigate]);

  /* Animated email placeholder */
  useEffect(() => {
    const text = 'admin@qlarity.com';
